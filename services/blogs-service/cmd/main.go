...
func main() {
	...
	blogRepository := repository.NewBlogPostRepository(database)
	blogUseCase := usecase.NewBlogPostUseCase(blogRepository)

	router := gin.Default()
	handler.NewBlogPostHandler(router, blogUseCase)
